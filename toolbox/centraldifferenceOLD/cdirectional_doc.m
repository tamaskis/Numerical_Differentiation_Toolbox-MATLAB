%% |cdirectional|
% Directional derivative of a multivariate, scalar-valued function using 
...
%% Description
% |Dv = cdirectional(f,x0,v)| numerically evaluates the directional
% derivative of $f(\mathbf{x})$ at $\mathbf{x}=\mathbf{x}_{0}$ in the
% direction of $\mathbf{v}$ using the central difference approximation with
% a default relative step size of $h=\varepsilon^{1/3}$, where 
% $\varepsilon$ is the machine zero.
%
% |Dv = cdirectional(f,x0,v,h)| numerically evaluates the directional
% derivative of $f(\mathbf{x})$ at $\mathbf{x}=\mathbf{x}_{0}$ in the
% direction of $\mathbf{v}$ using the central difference approximation with 
% a user-specified relative step size $h$.
...
%           <td style="text-align:center"><TT>Dv</TT></td>
%           <td style="text-align:center"><img src="https://latex.codecogs.com/svg.latex?\inline&space;\nabla_{\mathbf{v}}f(\mathbf{x}_{0})=\nabla f(\mathbf{x}_{0})^{T}\mathbf{v}" title="" /></td>
